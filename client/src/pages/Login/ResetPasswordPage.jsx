...
export default function ResetPasswordPage() {
  ...
  const onSubmit = async (data) => {
    try {
      // 🟢 รอ Backend ทำ API: POST /api/reset-password
      const response = await fetch('http://localhost:6700/api/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ newPassword: data.password })
      });

      if (!response.ok) throw new Error("ไม่สามารถเปลี่ยนรหัสผ่านได้");
